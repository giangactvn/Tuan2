// chương trình sửa văn bản bằng dòng lệnh
...
int seek(char *buff3, int k, int *count1, char *path) 
{
	...
	while (i <= strlen(buffer))
	{
		...
		column++;
		column2 = column;
		// Nếu xuống dòng thì tăng giá trị new_line và đặt giá trị column = 0
		if (buffer[i] == '\n') 
		{
			col[x] = -1;
			x++;
		...
		j = 0;
		// So sánh chữ vừa đọc trong buffer và so sánh với từ cần tìm
	...
	// Ghi vào file các thao tác vừa làm
